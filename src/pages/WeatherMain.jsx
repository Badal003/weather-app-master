/**
 * WeatherMain Component
 * Displays the main weather information including current temperature,
 * weather description, and additional weather metrics (wind, humidity, pressure)
 */
		...
		<React.Fragment>
			...
			<section
				...
				<section className="rounded-3 shadow my-5 py-2 current-weather-assets brand-tertiary-color d-flex align-items-center justify-content-around">
					<WeatherMetric 
						icon={WindIcon}
						value={db.get("SUB_WEATHER_WIND_VALUE") || "2.90 m/s"}
						label="Wind"
						iconAlt="wind-icon"
					/>
					<WeatherMetric 
						icon={HumidityIcon}
						value={db.get("SUB_WEATHER_HUMIDITY_VALUE") || "98%"}
						label="Humidity"
						iconAlt="humidity-icon"
					/>
					<WeatherMetric 
						icon={PressureIcon}
						value={db.get("SUB_WEATHER_PRESSURE_VALUE") || "1000 hPa"}
						label="Pressure"
						iconAlt="pressure-icon"
					/>
				</section>
				<Footer />
			</section>
